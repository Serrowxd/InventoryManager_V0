...
export function Sidebar({ currentPage, onNavigate, onOpenChatbot, onOpenHelp }: SidebarProps) {
  ...
  const actionItems = [
    { icon: Bot, label: "AI Assistant", action: onOpenChatbot },
      ...
      <div className="flex flex-col space-y-2 mt-auto">
        {actionItems.map((item, index) => (
          <Button
            ...
            variant="ghost"
            size="icon"
            className="w-12 h-12 rounded-xl text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-all duration-200"
            onClick={item.action}
            title={item.label}
          >
            <item.icon className="w-5 h-5" />
